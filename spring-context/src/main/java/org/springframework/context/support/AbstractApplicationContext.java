...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	// 该锁是用来同步applicationContext的刷新和销毁的
	 ...
	 * 根据环境，初始化占位符的。默认不做任何事情。GenericApplicationContext也没有覆盖。
	 ...
	 *
	 * 配置beanFactory，设置ClassLoader，bean后置处理器
	 */
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置属性编辑器PropertyEditor的注册器(用于类型转换等)
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		// 在工厂的beanPostProcessor属性中添加后置处理器
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		// 在自动装配（default-autowire）时，忽略指定接口的实现类中，和接口setter方法入参类型相同的对外的依赖。即，这些类的实现类的依赖在自动装配时被忽略
		...
		// 设置beanFactory的resolvableDependencies属性。将自身BeanFactory/ApplicationContext对象记录进resolvableDependencies属属性中，使自身对象可以被注入
	 ...
	 *
	 *
	 * 注册后置处理器，此时所有的bean definitions已经被加载，但是还没有bean被实例化。
	 */
	 ...
	 * 实例化并调用所有的bean factory后置处理器
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 实例化并调用所有已注册的BeanFactoryPostProcessor
		// getBeanFactoryPostProcessors方法：拿到当前应用上下文中已经注册的 BeanFactoryPostProcessor
		PostProcessorRegistrationDelegate.invokeBeanFactoryPostProcessors(beanFactory, getBeanFactoryPostProcessors());

		// Detect a LoadTimeWeaver and prepare for weaving, if found in the meantime
		// (e.g. through an @Bean method registered by ConfigurationClassPostProcessor)
		if (beanFactory.getTempClassLoader() == null && beanFactory.containsBean(LOAD_TIME_WEAVER_BEAN_NAME)) {
			// AOP 相关的 BeanPostProcessor
			beanFactory.addBeanPostProcessor(new LoadTimeWeaverAwareProcessor(beanFactory));
			beanFactory.setTempClassLoader(new ContextTypeMatchClassLoader(beanFactory.getBeanClassLoader()));
		}
	}
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 类型为ConversionService、名称为conversionService 的 bean 是一个转换服务对象
